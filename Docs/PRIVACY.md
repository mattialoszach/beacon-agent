...
- While a visible guide is waiting for an automatically verifiable outcome, Beacon may take one local OCR capture only when Accessibility evidence shows the expected control transition but the control has no usable label. This verification capture requires Screen Recording permission, is disabled for excluded applications, is redacted in memory, and is never sent to a model.
- Browser menu, settings, and account/profile guides, menu-bar navigation, and known static-menu Electron applications such as VS Code may use a fresh local reasoning capture after each explicitly initiated navigation step when Screen Recording is granted. This is used to find the next visible control when Accessibility labels are incomplete or the tree does not change; exclusions, in-memory redaction, bounded resolution, and the separate cloud-vision consent boundary still apply. Without Screen Recording, a matching menu-open notification and local `AXSelected` state can still advance the highlighted menu step; only the short role/label is retained in memory.
